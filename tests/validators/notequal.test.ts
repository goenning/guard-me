...
describe("NotEqual Validator", function() {
  ...
  fail(`"Star Wars" == "Star Wars"`, "Star Wars", notEqual("Star Wars"), [
    { property: "value", message: `Value should not be equal to Star Wars` }
  ])

  fail(`1 == 1`, 1, notEqual(1), [
    { property: "value", message: `Value should not be equal to 1` }
  ])

  fail(`1.2 == 1.2`, 1.2, notEqual(1.2), [
    { property: "value", message: `Value should not be equal to 1.2` }
  ])

  fail(`{ bar:"bar" } == { bar:"bar" }`, { bar: "bar" }, notEqual({ bar: "bar" }), [
    { property: "value", message: `Value should not be equal to {"bar":"bar"}` }
  ])
